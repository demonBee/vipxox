...
@property(nonatomic,strong)UIView *bgView;  //背景的颜色
...
#warning 颜色
    if (_xcolor==0) {
        _bgView.backgroundColor=ManColor;
        
    }else if (_xcolor==1){
        _bgView.backgroundColor=YooPink;
        
    }else if (_xcolor==2){
        _bgView.backgroundColor=BoyColor;
        
    }else if (_xcolor==3){
        _bgView.backgroundColor=NorthAmercia;
    }
...
- (void)initTitleView
{
   _bgView = [[UIView alloc] initWithFrame:CGRectMake(0,0,kDeviceWidth, 64)];
//    bgView.backgroundColor = [UIColor colorWithRed:62.0/255 green:199.0/255 blue:153.0/255 alpha:1.0];
//    _bgView.backgroundColor=[UIColor redColor];
    [self.view addSubview:_bgView];
    
    UILabel *titleLab = [[UILabel alloc] initWithFrame:CGRectMake((kDeviceWidth - ACTUAL_WIDTH(60)) / 2.0, 28, ACTUAL_WIDTH(60), 20)];
...
- (void)setOverlayPickerView
{
    ...
    //四个边角QRCodeTopLeft
    UIImage *cornerImage = [UIImage imageNamed:@""];
...
//    cornerImage = [UIImage imageNamed:@"QRCodeTopRight"];
    ...
    cornerImage = [UIImage imageNamed:@""];
    ...
    scanCropView.layer.borderColor = ManColor.CGColor;
