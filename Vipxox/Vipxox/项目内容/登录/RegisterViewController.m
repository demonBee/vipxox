...
-(void)creatView{
   ...
   _consumerNameTextField=[[UITextField alloc]initWithFrame:CGRectMake(ACTUAL_WIDTH(19), ACTUAL_HEIGHT(2), ACTUAL_WIDTH(317), ACTUAL_HEIGHT(36))];
    ...
    //“确认密码”
    UIView *sureSecretCodeView=[[UIView alloc]initWithFrame:CGRectMake(ACTUAL_WIDTH(20), ACTUAL_HEIGHT(336), ACTUAL_WIDTH(336), ACTUAL_HEIGHT(40))];
   ...
   _sureSecretCodeTextField=[[UITextField alloc]initWithFrame:CGRectMake(ACTUAL_WIDTH(19), ACTUAL_HEIGHT(2), ACTUAL_WIDTH(317), ACTUAL_HEIGHT(36))];
    ...
    //“请输入您要的注册邮箱地址”
    UILabel *promptLabel=[[UILabel alloc]initWithFrame:CGRectMake(ACTUAL_WIDTH(112), ACTUAL_HEIGHT(460), ACTUAL_WIDTH(154), ACTUAL_HEIGHT(20))];
...
//    [self dismissViewControllerAnimated:YES completion:nil];
    ...
    if (!_canSave) {
        ...
        //注册接口
//        http://www.vipxox.cn/? m=appapi&s=register&zt=2&email=&user=&pwd=&code=
        NSString*urlStr=[NSString stringWithFormat:@"%@",HTTP_ADDRESS];
        NSDictionary*params=@{@"m":@"appapi",@"s":@"register",@"email":self.mailBoxTextField.text,@"user":self.consumerNameTextField.text,@"pwd":self.secretCodeTextField.text,@"code":self.testCodeTextField.text,@"zt":@"2"};
        HttpManager*manager=[[HttpManager alloc]init];
        [manager getDataFromNetworkWithUrl:urlStr parameters:params compliation:^(id data, NSError *error) {
            ...
            if ([data[@"errorCode"] isEqualToString:@"0"]) {
                ...
                [Nsuser setObject:self.consumerNameTextField.text forKey:AUTOLOGIN];
